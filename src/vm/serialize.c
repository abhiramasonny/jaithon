...
ObjFunction *jaiDeserializeModule(const uint8_t *data, size_t size,
    ...
    if (curU32(&c) != JAI_BUILD_ID) return NULL;
...
/* ------------------------------------------------------------------ */
/* Differential comparison (--bootstrap-verify)                         */
/* ------------------------------------------------------------------ */

typedef struct {
    char  *buf;
    size_t size;
    bool   written;   /* only the first difference is reported */
} DiffSink;

static void diffSet(DiffSink *d, const char *fmt, ...) JAI_PRINTF(2, 3);

static void diffSet(DiffSink *d, const char *fmt, ...) {
    if (d == NULL || d->buf == NULL || d->size == 0 || d->written) return;

    va_list args;
    va_start(args, fmt);
    (void)vsnprintf(d->buf, d->size, fmt, args);
    va_end(args);
    d->written = true;
}

static const char *constKindName(Value v) {
    switch (v.type) {
    ...
    Obj *o = AS_OBJ(v);
    return o == NULL ? "null" : jaiObjTypeName(o->type);
}

static bool cmpFunctions(const ObjFunction *a, const ObjFunction *b,
                         DiffSink *d, const char *scope, int depth);

/* Bitwise, not numeric: two constant pools that disagree about -0.0 or about
 * int versus float are not the same pool. */
static bool sameDouble(double x, double y) {
    return memcmp(&x, &y, sizeof x) == 0;
}

static bool cmpConstant(Value x, Value y, DiffSink *d, const char *scope,
                        int index, int depth) {
    if (x.type != y.type) {
        diffSet(d, "%s: constant %d has type %s here and %s there", scope, index,
                constKindName(x), constKindName(y));
        return false;
    }
    ...
    case VAL_NULL:
        return true;
    case VAL_BOOL:
        if (AS_BOOL(x) == AS_BOOL(y)) return true;
        diffSet(d, "%s: constant %d differs: %s vs %s", scope, index,
                AS_BOOL(x) ? "true" : "false", AS_BOOL(y) ? "true" : "false");
        return false;
    case VAL_INT:
        if (AS_INT(x) == AS_INT(y)) return true;
        diffSet(d, "%s: constant %d differs: %lld vs %lld", scope, index,
                (long long)AS_INT(x), (long long)AS_INT(y));
        return false;
    case VAL_FLOAT:
        if (sameDouble(AS_FLOAT(x), AS_FLOAT(y))) return true;
        diffSet(d, "%s: constant %d differs: %g vs %g", scope, index,
    ...
    Obj *ox = AS_OBJ(x), *oy = AS_OBJ(y);
    if (ox == oy) return true;
    if (ox == NULL || oy == NULL || ox->type != oy->type) {
        diffSet(d, "%s: constant %d has type %s here and %s there", scope, index,
                constKindName(x), constKindName(y));
        return false;
    }
    if (depth > JAIC_MAX_DEPTH) {
        diffSet(d, "%s: constant %d nests deeper than %d levels", scope, index,
                JAIC_MAX_DEPTH);
        return false;
    }
    ...
    switch (ox->type) {
    case OBJ_STRING: {
        const ObjString *sx = (const ObjString *)ox, *sy = (const ObjString *)oy;
        if (sx->length == sy->length &&
            memcmp(sx->chars, sy->chars, sx->length) == 0) {
            return true;
        }
        diffSet(d, "%s: constant %d differs: string \"%.32s\" vs \"%.32s\"",
                scope, index, sx->chars, sy->chars);
        return false;
    }
    case OBJ_BYTES: {
        const ObjBytes *bx = (const ObjBytes *)ox, *by = (const ObjBytes *)oy;
        if (bx->length == by->length &&
            memcmp(bx->data, by->data, bx->length) == 0) {
            return true;
        }
        diffSet(d, "%s: constant %d differs: %u vs %u bytes", scope, index,
                (unsigned)bx->length, (unsigned)by->length);
        return false;
    }
    case OBJ_TUPLE: {
        const ObjTuple *tx = (const ObjTuple *)ox, *ty = (const ObjTuple *)oy;
        if (tx->count != ty->count) {
            diffSet(d, "%s: constant %d differs: tuple of %u vs %u", scope, index,
                    (unsigned)tx->count, (unsigned)ty->count);
            return false;
        }
        char inner[192];
        snprintf(inner, sizeof inner, "%s > constant %d", scope, index);
        for (uint32_t i = 0; i < tx->count; i++) {
            if (!cmpConstant(tx->items[i], ty->items[i], d, inner, (int)i,
                             depth + 1)) {
                return false;
            }
        }
    ...
    case OBJ_FUNCTION: {
        char inner[192];
        const ObjString *name = ((const ObjFunction *)ox)->name;
        snprintf(inner, sizeof inner, "%s > constant %d (%s)", scope, index,
                 name != NULL ? name->chars : "anonymous");
        return cmpFunctions((const ObjFunction *)ox, (const ObjFunction *)oy, d,
                            inner, depth + 1);
    }
    default:
        diffSet(d, "%s: constant %d is a %s, which has no structural comparison",
                scope, index, jaiObjTypeName(ox->type));
        return false;
    }
}

/* Start offset of the instruction containing byte `target`, by decoding from
 * zero. Falls back to the byte itself if the stream stops tiling. */
static int instructionStart(const Chunk *chunk, int target) {
    for (int off = 0; off < chunk->count;) {
        uint8_t raw = chunk->code[off];
        if (raw >= OP_COUNT) break;
        OpCode op = (OpCode)raw;

        int size = jaiOpOperandSize(op);
        if (op == OP_CLOSURE) {
            if (off + 4 > chunk->count) break;
            uint32_t k = jaiReadU24(chunk->code + off + 1);
            if (k >= (uint32_t)chunk->constants.count) break;
            Value fn = chunk->constants.data[k];
            if (!IS_FUNCTION(fn)) break;
            size = 3 + 3 * (int)AS_FUNCTION(fn)->upvalueCount;
        }
        if (size < 0) break;

        int next = off + 1 + size;
        if (target < next) return off;
        if (next <= off) break;
        off = next;
    }
    return target;
...
static const char *opNameAt(const Chunk *chunk, int offset) {
    if (offset < 0 || offset >= chunk->count) return "<end of code>";
    return jaiOpName((OpCode)chunk->code[offset]);
}

static bool cmpCode(const Chunk *ca, const Chunk *cb, DiffSink *d,
                    const char *scope) {
    int common = ca->count < cb->count ? ca->count : cb->count;
    int i = 0;
    while (i < common && ca->code[i] == cb->code[i]) i++;

    if (i == common) {
        if (ca->count == cb->count) return true;
        diffSet(d, "%s: code length differs: %d vs %d bytes (identical up to "
                   "offset %d, %s vs %s)",
                scope, ca->count, cb->count, i, opNameAt(ca, i), opNameAt(cb, i));
        return false;
    }

    int at = instructionStart(ca, i);
    const char *nameA = opNameAt(ca, at);
    const char *nameB = opNameAt(cb, at);
    if (ca->code[at] != (at < cb->count ? cb->code[at] : 0xFFu)) {
        diffSet(d, "%s: code differs at offset %d: %s vs %s", scope, at, nameA,
                nameB);
    } else {
        diffSet(d, "%s: code differs at offset %d: %s vs %s, operand byte %d is "
                   "0x%02x vs 0x%02x",
                scope, at, nameA, nameB, i - at - 1, (unsigned)ca->code[i],
                (unsigned)cb->code[i]);
    }
    return false;
}
...
static bool cmpFunctions(const ObjFunction *a, const ObjFunction *b,
                         DiffSink *d, const char *scope, int depth) {
    if (depth > JAIC_MAX_DEPTH) {
        diffSet(d, "%s: nests deeper than %d levels", scope, JAIC_MAX_DEPTH);
        return false;
    }
    if (a->arity != b->arity) {
        diffSet(d, "%s: arity differs: %u vs %u", scope, (unsigned)a->arity,
                (unsigned)b->arity);
        return false;
    }
    if (a->flags != b->flags) {
        diffSet(d, "%s: flags differ: 0x%03x vs 0x%03x", scope,
                (unsigned)a->flags, (unsigned)b->flags);
        return false;
    }
    if (a->maxSlots != b->maxSlots) {
        diffSet(d, "%s: frame size differs: %u vs %u slots", scope,
                (unsigned)a->maxSlots, (unsigned)b->maxSlots);
        return false;
    }
    if (a->upvalueCount != b->upvalueCount) {
        diffSet(d, "%s: upvalue count differs: %u vs %u", scope,
                (unsigned)a->upvalueCount, (unsigned)b->upvalueCount);
        return false;
    }
    if (a->defaultCount != b->defaultCount) {
        diffSet(d, "%s: default count differs: %u vs %u", scope,
                (unsigned)a->defaultCount, (unsigned)b->defaultCount);
        return false;
    }
    for (uint8_t i = 0; i < a->defaultCount; i++) {
        uint32_t x = a->defaultOffsets != NULL ? a->defaultOffsets[i] : 0;
        uint32_t y = b->defaultOffsets != NULL ? b->defaultOffsets[i] : 0;
        if (x == y) continue;
        diffSet(d, "%s: default thunk %u starts at %u vs %u", scope,
                (unsigned)i, (unsigned)x, (unsigned)y);
        return false;
    }

    if (!cmpCode(&a->chunk, &b->chunk, d, scope)) return false;

    if (a->chunk.constants.count != b->chunk.constants.count) {
        diffSet(d, "%s: constant count differs: %d vs %d", scope,
                a->chunk.constants.count, b->chunk.constants.count);
        return false;
    }
    for (int i = 0; i < a->chunk.constants.count; i++) {
        if (!cmpConstant(a->chunk.constants.data[i], b->chunk.constants.data[i],
                         d, scope, i, depth)) {
            return false;
        }
    }
    ...
    if (a->exceptionCount != b->exceptionCount) {
        diffSet(d, "%s: exception table differs: %u vs %u entries", scope,
                (unsigned)a->exceptionCount, (unsigned)b->exceptionCount);
        return false;
    }
    for (uint16_t i = 0; i < a->exceptionCount; i++) {
        const ExceptionEntry *x = &a->exceptions[i], *y = &b->exceptions[i];
        if (x->start == y->start && x->end == y->end &&
            x->handler == y->handler && x->typeConst == y->typeConst) {
            continue;
        }
        diffSet(d, "%s: exception entry %u differs: [%u,%u)->%u type %u vs "
                   "[%u,%u)->%u type %u",
                scope, (unsigned)i, (unsigned)x->start, (unsigned)x->end,
                (unsigned)x->handler, (unsigned)x->typeConst, (unsigned)y->start,
                (unsigned)y->end, (unsigned)y->handler, (unsigned)y->typeConst);
        return false;
    }
    return true;
}
...
bool jaiCompareFunctions(const ObjFunction *a, const ObjFunction *b,
    ...
    DiffSink sink = { diff, diffSize, false };
    if (diff != NULL && diffSize > 0) diff[0] = '\0';

    if (a == NULL || b == NULL) {
        diffSet(&sink, "missing function record: %s vs %s",
                a != NULL ? "present" : "null", b != NULL ? "present" : "null");
        return false;
    }

    /* Names are ignored for equality but make the message locatable. */
    char scope[192];
    snprintf(scope, sizeof scope, "%s",
             a->name != NULL ? a->name->chars : "<anonymous>");
    return cmpFunctions(a, b, &sink, scope, 0);
}
